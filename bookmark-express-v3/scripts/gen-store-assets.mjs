...
async function renderPopups() {
    ...
    await page.addInitScript(({ tree, counts }) => {
        const store = { bookmarkData: null, hash: null }
        window.chrome = {
    ...
    await page.fill('.search-bar', 'git')
    await page.waitForSelector('li')
    await page.waitForTimeout(200)
    let box = await page.evaluate(() => ({ width: Math.ceil(document.body.getBoundingClientRect().width), height: Math.ceil(document.body.scrollHeight) }))
    const results = await page.screenshot({ clip: { x: 0, y: 0, width: box.width, height: box.height } })

    await page.fill('.search-bar', '')
    await page.waitForTimeout(150)
    box = await page.evaluate(() => ({ width: Math.ceil(document.body.getBoundingClientRect().width), height: Math.ceil(document.body.scrollHeight) }))
    const empty = await page.screenshot({ clip: { x: 0, y: 0, width: box.width, height: box.height } })
...
async function screenshotResults(results) {
    const W = 1280, H = 800
    const card = await roundedCard(results, 1000, 22)
    const cardX = Math.round((W - card.w) / 2), cardY = 300
    const shadow = await softShadow(card.w, card.h, 26, 44, 0.5)
    const text = Buffer.from(`<svg width="${W}" height="${H}"><text x="${W / 2}" y="150" text-anchor="middle" font-family="'DejaVu Sans', Arial, sans-serif" font-weight="bold" font-size="52" fill="${CREAM}">Find any bookmark in a keystroke</text><text x="${W / 2}" y="205" text-anchor="middle" font-family="'DejaVu Sans', Arial, sans-serif" font-size="26" fill="${MUTED}">Open the popup and just start typing — results filter instantly.</text></svg>`)
...
async function screenshotLaunch(empty) {
    ...
    const text = Buffer.from(`<svg width="${W}" height="${H}"><text x="${W / 2}" y="150" text-anchor="middle" font-family="'DejaVu Sans', Arial, sans-serif" font-weight="bold" font-size="52" fill="${CREAM}">Launch from anywhere</text><text x="${W / 2}" y="205" text-anchor="middle" font-family="'DejaVu Sans', Arial, sans-serif" font-size="26" fill="${MUTED}">Press the shortcut, then type. No mouse required.</text>${capsSvg}</svg>`)
    await baseCanvas(W, H).composite([
        { input: shadow, left: cardX - 44, top: cardY - 44 + 14 },
        { input: card.buf, left: cardX, top: cardY },
        { input: text, left: 0, top: 0 },
